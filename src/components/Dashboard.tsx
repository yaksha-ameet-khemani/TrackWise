...
export default function Dashboard({ entries, onEdit }: Props) {
  ...
  const clients = new Set(entries.map((e) => e.client)).size;
  const pending = active.filter((e) =>
    ['Under Review by client', 'Pending'].includes(e.status)
  ).length;
  const approved = active.filter(
    (e) => e.status === 'Approved by client'
  ).length;
  ...
  const clientCounts = active.reduce<Record<string, number>>((acc, e) => {
    acc[e.client] = (acc[e.client] ?? 0) + 1;
    return acc;
  }, {});
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-5 gap-3 mb-6">
        <MetricCard label="Active Entries" value={total} sub="Currently tracked" />
        <MetricCard label="Clients" value={clients} sub="Unique clients" />
        <MetricCard label="Pending Review" value={pending} sub="Awaiting decision" />
        <MetricCard label="Approved" value={approved} sub="Client approved" />
        <MetricCard label="Replaced Qs" value={replaced} sub="Historical swaps" />
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                {[
                  'Date','Client','Program','Track','Skill',
                  'Question Shared','Type','Milestone','CSDM','Status',
                ].map((h) => (
                  <th
                    key={h}
            ...
            <tbody>
              {recent.map((e) => (
                ...
                >
                  ...
                  <td className="px-3 py-2 max-w-[80px] truncate text-gray-600" title={e.programName}>
                    {e.programName}
                  </td>
                  <td className="px-3 py-2 max-w-[100px] truncate text-gray-600" title={e.trackName}>
                    {e.trackName}
                  </td>
                  <td className="px-3 py-2 whitespace-nowrap">{e.skill}</td>
                  <td className="px-3 py-2 max-w-[180px] truncate" title={e.questionShared}>
                    {e.questionShared}
                  </td>
                  <td className="px-3 py-2 text-gray-600">{e.type}</td>
                  <td className="px-3 py-2 whitespace-nowrap text-gray-600">{e.milestone}</td>
                  <td className="px-3 py-2 whitespace-nowrap text-gray-600">{e.csdm}</td>
