...
import { CLIENTS, SKILLS, STATUSES } from "../constants";
...
const TH =
  "text-center px-3 py-2 text-xs font-medium text-gray-500 bg-gray-50 border border-gray-200";
...
export default function AllEntries({
  ...
  const questionSpans = computeSpans(filled, (e) => k(sk(e), e.questionShared));
    ...
    <div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        ...
        <div className="px-4 py-3 border-t border-gray-100 flex flex-wrap items-center gap-3">
          ...
          {totalRecords > 0 &&
            (() => {
              const mfa = filtered.filter((e) => e.type === "MFA").length;
              const sf = filtered.filter((e) => e.type === "SF").length;
              const mcq = filtered.filter((e) => e.type === "MCQ").length;
              const assessment = filtered.filter((e) =>
                e.milestone.toLowerCase().includes("assessment"),
              ).length;
              const mock = filtered.filter((e) =>
                e.milestone.toLowerCase().includes("mock"),
              ).length;
              const actual = filtered.filter((e) =>
                e.milestone.toLowerCase().includes("actual"),
              ).length;
              const badge = (label: string, count: number, color: string) => (
                <span
                  key={label}
                  className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-medium border ${color}`}
                >
              ...
              return (
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {sf > 0 &&
                    badge("SF", sf, "bg-blue-50 text-blue-700 border-blue-200")}
                  {mcq > 0 &&
                    badge(
                      "MCQ",
                      mcq,
