  ...
  return (
    <div className="relative flex items-center justify-center w-24 h-24 flex-shrink-0">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-xl font-bold text-gray-900 leading-none">{centerValue}</span>
        <span className="text-[9px] text-gray-400 mt-0.5 leading-tight text-center">Data Shared</span>
      </div>
    </div>
  );
}
...
function MilestoneColumns({
  ...
  segments: { label: string; count: number; textColor: string; barColor: string }[];
}) {
  return (
    <div className="flex items-stretch gap-0 flex-1 justify-center">
      {segments.map((s, i) => (
        <div key={s.label} className="flex items-stretch">
          ...
          {i > 0 && (
            <div className="w-px bg-black/5 mx-6 self-stretch" />
          )}
          <div className="flex flex-col items-center justify-center gap-1 px-2">
            <span className="text-2xl font-extrabold" style={{ color: s.barColor }}>
              {s.count}
            </span>
...
function CategoryCard({
  ...
  milestoneSegments: { label: string; count: number; color: string; textColor: string; barColor: string }[];
}) {
  const total = rows.length;
  const accepted = rows.filter((e) => e.status !== "Feedback Received" && e.status !== "Rejected").length;
  const feedback = rows.filter((e) => e.status === "Feedback Received").length;
  const rejected = rows.filter((e) => e.status === "Rejected").length;

  return (
    <div className={`rounded-2xl border ${accentBorder} shadow-sm overflow-hidden`}
      style={{ background: `linear-gradient(135deg, ${donutTrack} 0%, #ffffff 60%)` }}>
        ...
        {total === 0 ? (
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">No entries for the selected period.</p>
            </div>
          </div>
        ) : (
          <div className="flex items-center gap-6">
            ...
            <div className="w-72 flex-shrink-0">
              ...
              {/* Single subtitle row */}
              <p className="text-xs text-gray-500 leading-relaxed pl-1 whitespace-nowrap">
            ...
            {/* CENTER — donut */}
            <DonutChart
              segments={milestoneSegments.map((s) => ({ count: s.count, color: s.barColor }))}
              total={total}
              centerValue={total}
            />
...
function KpiCard({
  ...
  return (
    <div className={`flex-1 min-w-[160px] rounded-2xl p-5 ${gradient}`}>
      ...
      <p className="text-xs font-semibold opacity-70 mb-1 text-white">{label}</p>
  ...
  return (
    <div className="flex-1 min-w-[180px] bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-start gap-3">
      ...
      <div className="min-w-0">
        ...
        <p className="text-sm font-bold text-gray-900 leading-snug mt-0.5">{value}</p>
...
function normalizeMilestone(m: string): "Assessment" | "Assignment" | "Review / Demo" {
  const s = (m || "").trim();
...
function milestoneCounts(rows: Entry[]) {
  const accepted = rows.filter((e) => e.status !== "Feedback Received" && e.status !== "Rejected");
...
export default function UpdatedDashboard({ entries }: Props) {
  ...
  const totalAccepted = base.filter((e) => e.status !== "Feedback Received" && e.status !== "Rejected").length;
  const totalRejected = base.filter((e) => e.status === "Rejected").length;
  const acceptanceRate = totalShared > 0 ? ((totalAccepted / totalShared) * 100).toFixed(1) : "0.0";
  const rejectionRate = totalShared > 0 ? ((totalRejected / totalShared) * 100).toFixed(1) : "0.0";
  ...
  const projectSegments = [
    { label: "Assessment",    count: mfCounts["Assessment"],    color: "bg-violet-100",  textColor: "text-violet-700", barColor: "#5b21b6" },
    { label: "Assignment",    count: mfCounts["Assignment"],    color: "bg-pink-100",   textColor: "text-pink-700",   barColor: "#db2777" },
    { label: "Review / Demo", count: mfCounts["Review / Demo"], color: "bg-orange-100", textColor: "text-orange-700", barColor: "#ea580c" },
  ];

  const codingSegments = [
    { label: "Assessment",    count: sfCounts["Assessment"],    color: "bg-blue-100",   textColor: "text-blue-700",  barColor: "#1e40af" },
    { label: "Assignment",    count: sfCounts["Assignment"],    color: "bg-indigo-100",  textColor: "text-indigo-700",  barColor: "#4f46e5" },
    { label: "Review / Demo", count: sfCounts["Review / Demo"], color: "bg-amber-100",   textColor: "text-amber-700",  barColor: "#d97706" },
  ];

  const saSegments = [
    { label: "Assessment",    count: saCounts["Assessment"],    color: "bg-teal-100",   textColor: "text-teal-700",  barColor: "#0d9488" },
    { label: "Assignment",    count: saCounts["Assignment"],    color: "bg-emerald-100",textColor: "text-emerald-700",barColor: "#10b981" },
  ];

  const allAccepted = base.filter((e) => e.status !== "Feedback Received" && e.status !== "Rejected");
  const assessmentCount = allAccepted.filter((e) => normalizeMilestone(e.milestone || "") === "Assessment").length;
  const assessmentPct = allAccepted.length > 0 ? ((assessmentCount / allAccepted.length) * 100).toFixed(0) : "0";
  const saActivityPct = totalShared > 0 ? ((saRows.length / totalShared) * 100).toFixed(0) : "0";
  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 mb-7">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Content Performance Overview</h1>
          <p className="text-sm text-gray-400 mt-0.5">Summary of shared content and its status across categories</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 bg-white border border-gray-200 rounded-xl px-3 py-1.5 shadow-sm">
            ...
            <input
              ...
              className="text-xs text-gray-700 bg-transparent focus:outline-none w-32"
              title="From date"
            />
          ...
          {hasFilter && (
            <button
              onClick={() => { setDateFrom(""); setDateTo(""); setClient(""); }}
        ...
        <KpiCard
          ...
          icon={
            <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" strokeWidth={2.2} viewBox="0 0 24 24">
              <rect x="3" y="3" width="18" height="18" rx="4" fill="currentColor" stroke="none" opacity="0.25" />
      ...
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-semibold text-gray-700 tracking-wide uppercase">Category Performance</h2>
      </div>
      ...
      <div className="flex flex-col gap-3 mb-7">
        ...
        <CategoryCard
          ...
          icon={
            <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
              <path d="M8 9l-3 3 3 3M16 9l3 3-3 3M12 6l-2 12" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          }
      ...
      {/* ── Key Insights ── */}
      <div>
        <h2 className="text-sm font-semibold text-gray-700 tracking-wide uppercase mb-3">Key Insights</h2>
